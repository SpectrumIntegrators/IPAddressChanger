...
internal static class NetworkManager {
	...
	private static List<AdapterInfo> GetAdapters() {
		...
		foreach (CimInstance ci in session.QueryInstances(Namespace, "WQL", "SELECT * FROM MSFT_NetAdapter")) {
			using (ci) {
				...
				uint operational = p["InterfaceOperationalStatus"]?.Value as uint? ?? 0;
				uint admin = p["InterfaceAdminStatus"]?.Value as uint? ?? 0;
				list.Add(new AdapterInfo(
					p["InterfaceIndex"]?.Value as uint? ?? 0,
					p["Name"]?.Value?.ToString() ?? "<unknown>",
					p["DriverDescription"]?.Value?.ToString() ?? "<unknown>",
					operational == 1,
					admin == 1,
					p["Speed"]?.Value as ulong? ?? 0,
		...
		foreach (CimInstance ci in session.QueryInstances(Namespace, "WQL", query)) {
			using (ci) {
				...
				list.Add(new IPAddressInfo {
					AddressFamily = p["AddressFamily"]?.Value as ushort? ?? 0,
	...
	private static Dictionary<uint, List<IPAddressInfo>> GetAllIPAddresses() {
		...
		foreach (CimInstance ci in session.QueryInstances(Namespace, "WQL", "SELECT * FROM MSFT_NetIPAddress")) {
			using (ci) {
				var p = ci.CimInstanceProperties;
				uint idx = p["InterfaceIndex"]?.Value as uint? ?? 0;
	...
	private static void NewIPAddress(uint interfaceIndex, string ipAddress, byte prefixLength) {
		...
		uint returnCode = result.ReturnValue?.Value as uint? ?? 0;
		if (returnCode != 0) {
	...
	private static uint RenewDhcp(uint interfaceIndex) {
		...
		foreach (CimInstance config in session.QueryInstances(LegacyNamespace, "WQL", query)) {
			using (config) {
				...
				return result.ReturnValue?.Value as uint? ?? 0xFFFFFFFF;
			}
		}
